...
def download_video(url):
    try:
        ...
        command = [
            'yt-dlp',
            '--no-playlist',
            '-f', 'bestvideo[ext=mp4]+bestaudio[ext=m4a]/best',  # Highest quality
...
def start_handler(message):
    ...
    if message.from_user.id == ADMIN_ID:
        ...
        welcome_text = f"👋 Salom {username}!\n\nSend me any video link (TikTok, YouTube, Instagram) and download!"
...
def handle_url(message):
    ...
    if url.startswith('http'):
        wait_msg = bot.send_message(message.chat.id, "⏳ Downloading your video, please wait...")

        title = get_video_title(url)
        caption = title if title else "🎬 Here is your video!"
