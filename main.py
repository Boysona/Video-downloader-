...
def normalize_short_url(url: str) -> str:
    # Add custom normalization here if needed
    return url
...
@bot.message_handler(func=lambda m: re.search(URL_REGEX, m.text or "") and is_youtube_url(m.text), content_types=['text'])
def handle_youtube_url(message):
    ...
    try:
        ...
        logging.info(f"Attempting to download video from: {url} with options: {ydl_opts}")
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        ...
        if transcription:
            if len(transcription) > 2000:
                ...
                bot.send_message(message.chat.id, f"📝 Sharaxaada Muuqaalka (Qoraalka Whisper):\n\n{transcription}")
        else:
            bot.send_message(message.chat.id, "⚠️ Ma awoodo in aan soo saaro sharaxaada muuqaalkaan.")

    except Exception as e:
        logging.error(f"Error downloading or processing video: {e}")
        bot.send_message(message.chat.id, "⚠️ Khalad ayaa dhacay intii aan soo dejinayay ama falanqaynayay muuqaalka.")
    finally:
        try:
...
def handle_social_video(msg):
    ...
    try:
        ...
        if video_path:
            ...
            bot.send_message(msg.chat.id, "⚠️ Ma awoodin inaan soo dejiyo muuqaalkan.")
    except Exception as e:
        bot.send_message(msg.chat.id, f"⚠️ Khalad ayaa dhacay intii aan soo dejinayay muuqaalka: {e}")
    finally:
...
def fallback(message):
    if not check_subscription(message.from_user.id):
        return send_subscription_message(message.chat.id)
    bot.send_message(message.chat.id, "⚠️ Fadlan soo dir cod, maqal, muuqaal ama video note. Haddii aad rabto inaad soo dejiso muuqaal, kaliya soo dir linkiga.")
