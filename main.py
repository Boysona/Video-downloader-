# Let's construct the updated and fixed version of the main.py file according to the user's requirements.
...
def normalize_short_url(url: str) -> str:
    try:
        ...
        logging.error(f"Redirect follow error: {e}")
        return url
    ...
    try:
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            try:
                ...
                bot.send_message(message.chat.id, f"⚠️ Khalad ayaa dhacay: {e}")
                return
            except Exception as e:
                logging.error(f"Unexpected error: {e}")
                bot.send_message(message.chat.id, "⚠️ Khalad lama filaan ah ayaa dhacay.")
                return
