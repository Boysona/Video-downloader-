Waayahay, waxaan fahmay inaad rabto in bot-kaaga uu u shaqeeyo si la mid ah code 2 marka la eego sida webhook-ga loo xirayo iyo sida linkiga webhook-ga loogu qoro. Waxaan ku tusi doonaa sida loo sameeyo isbeddelada lagama maarmaanka ah code 1.
Halkan waxaa ah code-kaaga 1 oo wax laga beddelay si uu ula mid noqdo code 2 habka webhook-ga loo xirayo iyo qoraalka linkiga webhook-ga:
import os
import re
import uuid
import shutil
...
# ——— CONFIG ———
TOKEN = "8136008912:AAHwM1ZBZ2WxgCnFpRA0MC_EIr9KcRQiF3c"  # Bot token directly written
ADMIN_ID = 5978150981
REQUIRED_CHANNEL = "@qolka_ka"
WEBHOOK_URL = "https://video-downloader-1pmv.onrender.com" # Ka saar haddii aad rabto habka code 2
...
@app.route('/set_webhook', methods=['GET', 'POST']) # Waxaan ku darnay 'POST'
def set_webhook():
    webhook_url = "https://video-downloader-1pmv.onrender.com" # Halkan ayaad ku qori doontaa linkiga webhook-ga
    if webhook_url:
...
@app.route('/delete_webhook', methods=['GET', 'POST']) # Waxaan ku darnay 'POST'
def del_webhook():
    bot.delete_webhook()
...
if __name__=="__main__":
    ...
    os.makedirs(DOWNLOAD_DIR, exist_ok=True)
    bot.delete_webhook()
    bot.set_webhook(url="https://video-downloader-1pmv.onrender.com") # Halkan ayaad sidoo kale ku qori doontaa linkiga webhook-ga
    app.run(host="0.0.0.0", port=int(os.getenv("PORT",8080)))

Isbeddelada aan sameeyay:
 * Ka saarida WEBHOOK_URL ee isku xidhka hore: Waxaan ka saarnay isku xidhka hore ee WEBHOOK_URL si aan ula mid noqono code 2 oo linkiga webhook-ga ku qoraya gudaha hawlaha /set_webhook iyo if __name__ == "__main__":.
 * Ku darida POST habka /set_webhook iyo /delete_webhook: Waxaan ku darnay POST hababka loo ogol yahay ee /set_webhook iyo /delete_webhook si ay ula mid noqdaan code 2.
 * Qoraalka linkiga webhook-ga gudaha /set_webhook: Waxaad hadda ku qori kartaa linkiga webhook-gaaga gudaha doorsoomaha webhook_url ee ku jira hawsha /set_webhook.
 * Qoraalka linkiga webhook-ga gudaha if __name__ == "__main__":: Waxaad sidoo kale ku qori kartaa linkiga webhook-gaaga gudaha bot.set_webhook() ee ku jira qaybta if __name__ == "__main__":.
Sida loo isticmaalo:
Markaad code-kan cusub socodsiiso, waxaad u baahan doontaa inaad booqato /set_webhook endpoint-ka barnaamijkaaga (tusaale ahaan, https://video-downloader-1pmv.onrender.com/set_webhook) si aad u dejiso webhook-ga. Waxaad sidoo kale ku qori kartaa linkiga webhook-gaaga si toos ah code-ka dhexdiisa sida ku xusan.
Fadlan la soco inaad u baahan doonto inaad ku beddesho "https://video-downloader-1pmv.onrender.com" linkiga webhook-gaaga dhabta ah.
Haddii aad qabtid wax su'aalo kale ah, waan diyaar ahay inaan ku caawiyo!
